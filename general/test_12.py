from solution_12 import DirFileHash
...
@pytest.fixture
def dir_with_files_long(tmp_path):
    d = tmp_path

    with open(d / 'ascii_lowercase', 'a') as f:
        for i in range(10):
            f.write(string.ascii_lowercase)

    with open(d / 'ascii_uppercase', 'a') as f:
        for i in range(10):
            f.write(string.ascii_uppercase)
...
@pytest.mark.nbm
def test_all_in_dir_long(dir_with_files_long):
    dfh = DirFileHash(dir_with_files_long)

    for one_filename in os.listdir(dir_with_files_long):
        m = md5()
        content = getattr(string, one_filename).encode()
        for i in range(10):
            m.update(content)
